//Emitir_relatorio_ATV_WF_insucesso_1.0.js

//Robô para emissão de relatório do WF da Claro utilizando senha de terceiros com foco no motivo dos Insucessos.
...
async function main() {
    ...
    await page.getByRole('checkbox', { name: 'Data Aprovação da Medição' }).check();
    console.log('Selecionando Data Aprovação da Medição');
    await page.waitForTimeout(500);
    ...
    await page.getByRole('checkbox', { name: 'Motivo de Insucesso' }).check();
    console.log('Selecionando Motivo de Insucesso');
    await page.waitForTimeout(500);
    ...
    // Calculando o último dia do mês anterior
        const hoje = new Date();
            const ultimoDiaObj = new Date(hoje.getFullYear(), hoje.getMonth(), 0);
        const ultimoDia = ultimoDiaObj.getDate().toString(); // Retorna "31", "30", "28" ou "29"

    // Selecionando o ultimo dia do mês anterio de forma dinâmica
